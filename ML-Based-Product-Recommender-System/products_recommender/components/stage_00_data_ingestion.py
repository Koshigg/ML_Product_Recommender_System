...
class DataIngestion:
    ...
    def download_data(self):
        ...
        try:
            ...
            # Kosh dataset_url = self.data_ingestion_config.dataset_download_url
            zip_download_dir = self.data_ingestion_config.raw_data_dir
            os.makedirs(zip_download_dir, exist_ok=True)
            # Kosh data_file_name = os.path.basename(dataset_url)
            data_file_name = "products_data.zip"
            zip_file_path = os.path.join(zip_download_dir, data_file_name)
            # Kosh logging.info(f"Downloading data from {dataset_url} into file {zip_file_path}")
            logging.info(f"Directly using the data from the raw data folder {zip_file_path}")
            # Kosh urllib.request.urlretrieve(dataset_url,zip_file_path)
